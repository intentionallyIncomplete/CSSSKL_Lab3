 ...
 * Passing a primitive allows modification to the value when the Object is constructed.
 * Passing an Object to a method grants access to that Object's attributes.
 * 
 * b. When a primitive is passed to a method, does the scope of that primitive change (i.e., grow to include the called method)?
 * If declared outside the method, the variable retains its original scope but the new value is local to the method unless
 * returned.
 * 
 * c. What about when an object is passed to a method?
 * If declared outside the method, the Object's attributes are available to pull data from, and the scope is retained.
 * If an Object is created or declared in a method or method signature then its scope is limited to that method.
 * 
 * d. How then would you describe the scope of an object that has been passed to a method?
 * The scope of an object passed to a method, if not declared elsewhere - has its scope then limited to that
 * method.
 * 
 * */
